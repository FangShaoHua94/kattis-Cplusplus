...
//  freopen("dijkstra_in.txt", "r", stdin);
  ...
  */
  ...
  priority_queue<ii, vector<ii>, greater<ii>> pq; pq.push({0, s});
	...
	//d-f.first u-f.second v-g.first w-g.second
	
  // sort the pairs by non-decreasing distance from s
  while (!pq.empty()) {                          // main loop
    auto f = pq.top(); pq.pop();            // shortest unvisited u
    if (f.first > dist[f.second]) continue;                   // a very important check
    for (auto &g : AL[f.second]) {                 // all edges from u
    	cout<<"distance compare:"<<dist[f.second]+g.second<<" vs "<<dist[g.first]<<endl;
      if (dist[f.second]+g.second >= dist[g.first]) continue;   
	  cout<<"["<<f.first<<","<<f.second<<"] ["<<g.first<<","<<g.second<<"]"<<endl;     // not improving, skip
      dist[g.first] = dist[f.second]+g.second;                       // relax operation
      pq.push({dist[g.first], g.first});                     // enqueue better pair
    }
  }
